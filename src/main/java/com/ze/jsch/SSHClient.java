...
public class SSHClient {
    ...
    private SSHClient(SSHExecutor.SSHInfo sshInfo) throws JSchException {
        jsch =new JSch();
        session = jsch.getSession(sshInfo.getUser(),sshInfo.getHost(),sshInfo.getPort());
        session.setPassword(sshInfo.getPassword());
        session.setUserInfo(new SSHClient.MyUserInfo());
        // 跳过鉴权的确认
     ...
     * */
    public static SSHClient newInstance() throws JSchException {
        SSHExecutor.SSHInfo sshInfo = new SSHExecutor.SSHInfo("root","root123","192.168.1.10",22);
        return new SSHClient(sshInfo);
    }
